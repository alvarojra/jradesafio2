...
if (formProduct != null) {
	formProduct.addEventListener("submit", (event) => {
		...
		returnMessage('success', "Cadastrado com sucesso");
...
function displayProducts() {
	...
	if(products) {
		...
		returnMessage('info', 'Nenhum Produto cadastrado no sistema.')
	}
}
...
if (formClient != null) {
	formClient.addEventListener("submit", (event) => {
		...
		if (fullname == ''  || city == '' || uf == '' || street == '' || cep == '') {
			returnMessage('warning', 'Ops, é necessário preencher todos os campos');
			return;
		}
...
function displayClients() {
	...
	if(clients) {
		...
		returnMessage('info', 'Nenhum cliente cadastrado no sistema.')
	}
}
